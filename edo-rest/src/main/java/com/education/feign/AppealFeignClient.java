...
@FeignClient(name = EDO_REPOSITORY_NAME, url = SERVICE_APPEAL_URL)
public interface AppealFeignClient extends BaseRestFeignClient<AppealDto> {
    ...
    ResponseEntity<AppealDto> upload(@RequestParam("id") Long id, @RequestParam("file") FilePoolDto file);
    ...
    AppealDto register(@RequestParam("id") Long id);
}
