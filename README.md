...
Для запуска сервера и установи пароля используйте следующую команду в консоли: 
...
Для перехода в хранилище иcпользуйте ссылку http://127.0.0.1:9000
...
После запуска сервисов переходим в томкат и проверяем методы:
1)	**checkConnection**
...
После чего нажимаем send для отправки запроса, в ответе должны получить код статуса 200, File uploaded. **(название файла в UUID формате)** type: **(формат файла)**
3)	**downloadFile**
...
переходим во вкладку Params и указываем следующие параметры:

- KEY: **type**
- VALUE: **(формат файла в виде doc, docx, png, jpeg, pdf)**
...
>http://localhost:.../api/rest/minio/upload

После чего переходим во вкладку body и указываем следующие пераметры
- KEY: **file**
- VALUE: **(файл, который мы хотим загрузить в форматах doc, docx, png, jpeg, pdf)**

После чего нажимаем send для отправки запроса, в ответе должны получить код статуса 200, File uploaded. **(название файла)** type: **(формат файла)**
...
>http://localhost:../api/rest/minio/download/(название файла в формате UUID)
...
>http://localhost:../api/rest/minio/download/(название файла в формате UUID)?type=doc

>http://localhost:../api/rest/minio/download/(название файла в формате UUID)?type=docx
3)	**delete** (удаление файла с корзины minIO)

Выбираем метод **DELETE** и указываем следующую ссылку: 

>http://localhost:.../api/rest/minio/delete/(**название файла в формате UUID**)
